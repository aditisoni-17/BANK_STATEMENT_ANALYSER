  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {cards.map((card) => (
        <article
          key={card.label}
          className={`rounded-xl bg-white p-5 shadow-sm ring-1 ${card.accent}`}
        >
          <p className="text-sm font-medium text-slate-500">{card.label}</p>
          <div className="mt-2 flex items-end justify-between gap-3">
            <strong className={`text-2xl font-semibold tracking-tight ${card.tone}`}>
              {card.value}
            </strong>
            <span className="text-xs font-medium text-slate-400">+10% from last analysis</span>
          </div>
        </article>
      ))}
    </div>
